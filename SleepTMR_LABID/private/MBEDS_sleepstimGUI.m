...
fig = figure('Name', ' TMR Sleep Stimulation  -  ' + S.subid, ...
...
stop_snd_btn = uicontrol('Style', 'pushbutton', 'String', 'Pause Stimulation', ...
    'Position', [50, 235, 150, 40], ...  
    'Callback', @stopSoundSeries);
stop_snd_btn.Enable = 'off';

% Label and Sound Volume
uicontrol('Style', 'text', ...
    'Position', [260, 40, 100, 20], ...
    'String', 'Sound Volume:', ...
    'HorizontalAlignment', 'center');

% default was previously hardcoded here; now lives in MBEDS_LabConfig
RES.soundVolume = S.default_sound_volume;
...
uicontrol('Style', 'text', ...
    'Position', [450, 40, 120, 20], ...
    'String', 'Background Volume:', ...
    'HorizontalAlignment', 'center');

% default was previously hardcoded here; now lives in MBEDS_LabConfig
RES.backgroundVolume = S.default_background_volume;
...
% incSound_btn = uicontrol('Style', 'pushbutton', 'String', 'Decrease Sound Volume', ...
%     'Position', [600, 185, 150, 30], ...
%     'Callback', @decreaseSoundVolume);

%%%
sndVolGroup = uibuttongroup('Units','pixels','Position',[30, 170, 110, 60],'Title','Sound Volume');

incSound_btn = uicontrol(sndVolGroup, 'Style', 'pushbutton', 'String', '-', ...
    'Position', [10, 10, 30, 30], ...
    'Callback', @decreaseSoundVolume);
incSound_btn = uicontrol(sndVolGroup, 'Style', 'pushbutton', 'String', '+', ...
    'Position', [60, 10, 30, 30], ...
    'Callback', @increaseSoundVolume);

bgVolGroup = uibuttongroup('Units','pixels','Position',[30, 100, 110, 60],'Title','Background Volume');
decSound_btn = uicontrol(bgVolGroup, 'Style', 'pushbutton', 'String', '-', ...
    'Position', [10, 10, 30, 30], ...
    'Callback', @decreaseBackgroundVolume);

incBg_btn = uicontrol(bgVolGroup, 'Style', 'pushbutton', 'String', '+', ...
    'Position', [60, 10, 30, 30], ...
    'Callback', @increaseBackgroundVolume);
%%%%

% Stepsize selector
stepSizeGroup = uibuttongroup('Units','pixels','Position',[155, 120, 70, 90],'Title','Vol. Stepsize');
uicontrol(stepSizeGroup,'Style','radiobutton','String','0.1','Position',[5,5,100,20],...
    'Callback',@(~,~)setStepSize(0.1));
uicontrol(stepSizeGroup,'Style','radiobutton','String','0.05','Position',[5,25,100,20],...
    'Callback',@(~,~)setStepSize(0.05));
uicontrol(stepSizeGroup,'Style','radiobutton','String','0.01','Position',[5,45,100,20],...
