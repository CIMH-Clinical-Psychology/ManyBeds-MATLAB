...
function C = MBEDS_LabConfig
    ...
    % Set force_value=1 or any value that should be send in this case. 
    ...
    C.noise_type = string(C.noise_type);
    C.language = string(C.language);
    C.location = string(C.location);
    C.lab_id = string(C.lab_id);
    if C.debug_mode
    ...
    if ~ ismember(C.trigger_interface, ["parallel", "serial"])
                error('trigger_interface must be "parallel" or "serial".');
    end
    if ~ismember(C.noise_type, ["white", "pink"])
